...
AgentX 即 Agent eXtension：一个统一的 CLI 工具，用于在多个 AI 编程代理之间管理 **MCP（Model Context Protocol）服务器** 与 **技能（Skills）**。

## 概览

AgentX 简化了在主流 AI 编程工具中安装、管理与监控 MCP 服务器与技能的流程：
...
### Claude Code 与 Codex 技能管理
- 从本地路径或 Git 仓库安装技能
- 支持技能目录（包含 `SKILL.md`）；Claude Code 还支持命令文件（`.md`）
